...
public:
    ...
    void jointState_cb(const control_msgs::JointTrajectoryControllerState& msg){
        double vels[4], poses[4] ;
        for (int i = 0; i < 4; i++) vels[i] = msg.desired.velocities[i];
        for (int i = 0; i < 4; i++) poses[i] = msg.desired.positions[i];
        double diffPoses[4] = { abs(poses[0]-steer_arr[0]), abs(poses[1]-steer_arr[1]),abs(poses[2]-steer_arr[2]),abs(poses[3]-steer_arr[3]) };
        Ddeg = find_max(diffPoses);
        max_vel = find_max(vels);
        //cout<<"max vel -->"<< max_vel<<"    dDeg -->"<< Ddeg << endl;
    }


    void steering_time(trajectory_msgs::JointTrajectoryPoint& steer_points){
        if(abs(Ddeg)>_eps){
        //cout<< Ddeg<< <<endl;
        ros::Duration duration= ros::Duration(1*Ddeg/Pi +_eps);
