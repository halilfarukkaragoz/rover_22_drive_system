...
public:
    odometry(int argc, char *argv[]){
        ...
        odomPub = nh.advertise<nav_msgs::Odometry>("drive_system/odometry_new_02",10);
        ...
        while (ros::ok)
        {
            ...
            current_time = ros::Time::now();
            double speed = find_l2_norm(x_speed,y_speed);

            if((speed >0.4 || speed < -0.4)  && (angular_z >0.3 || angular_z < -0.3)) // if move in a circular way 
            {
                double relx = Vmean[0];
                double rely = Vmean[1];
    ...
    void encoder_cb(const rover_22_drive_system::Steering& msg){
        encoder_msg = msg;
        for (int i = 0; i < 4; i++)encoder_msg.steering_angle[i] = (encoder_msg.steering_angle[i]-2048)/2048*Pi;      
    }
...
int main(int argc, char *argv[])
{
    ...
    ros::init(argc, argv, "Steering_odometry_new_02",ros::init_options::NoSigintHandler);
